...
class Exporter:
    ...
    def export_trades_csv(self, path: Optional[str] = None) -> str:
        ...
        if path is None:
            path = os.path.join(REPORTS_DIR, f"trades_{datetime.utcnow().date()}.csv")
    ...
    def export_pdf_report(self, path: Optional[str] = None) -> Optional[str]:
        ...
        if path is None:
            path = os.path.join(REPORTS_DIR, f"report_{datetime.utcnow().date()}.pdf")
        ...
        c.drawString(2 * cm, y, f"Trading-Report – {datetime.utcnow().date()}")
        y -= 1 * cm
    ...
    def _write_text_report(self, path, report, recent, exits):
        with open(path, "w", encoding="utf-8") as f:
            f.write(f"Trading-Report – {datetime.utcnow().date()}\n")
            f.write("=" * 50 + "\n\n")
            f.write("Accuracy:\n")
            for k, v in report.items():
                f.write(f"  {k}: {v}\n")
