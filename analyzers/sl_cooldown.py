...
Wenn eine Position durch einen VERLUSTIGEN Stop-Loss geschlossen wird
(ohne vorherigen Partial-TP), blockiert dieser Tracker den Kauf desselben
Tickers für SL_COOLDOWN_DAYS Tage.

Verdrahtung (seit Juli 2026, Pendant zur Backtest-Regel in
backtesting/engine.py):
  - record():     strategy/executor.py nach gebuchtem Clean-SL-Exit
  - is_blocked(): strategy/swing_strategy.py im Kauf-Gate
...
class StopLossCooldown:
    def __init__(self, cooldown_days: Optional[int] = None):
        if cooldown_days is None:
            from config import config
            cooldown_days = int(getattr(config, "sl_cooldown_days", 2))
