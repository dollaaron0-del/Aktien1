...
Umgebungsvariablen (.env):
  ...
  IBKR_PORT       = 7497           (TWS Live) | 7498 (TWS Paper) | 4001 (GW Live) | 4002 (GW Paper)
  IBKR_CLIENT_ID  = 1
  IBKR_ACCOUNT    = ""             (leer = erstes verfügbares Konto)
...
_ORDER_TIMEOUT   = 30   # Sekunden Wartezeit auf Fill
...
class IBKRBroker:
    ...
    def _connect(self) -> bool:
        ...
        try:
            ib = IB()
            ib.connect(_HOST, _PORT, clientId=_CLIENT_ID, timeout=10)
            self._ib = ib
            self._connected = True
            account = _ACCOUNT or (ib.managedAccounts()[0] if ib.managedAccounts() else "?")
            log.info("IBKR verbunden: %s:%d | Account: %s", _HOST, _PORT, account)
            return True
    ...
    def _place_order(self, contract, action: str, shares: float) -> Dict:
        ...
        log.info("IBKR %s %s %.4f shares – Order eingereicht", action, contract.symbol, shares)
